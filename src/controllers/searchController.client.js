...
function displayBusinesses(data) {
  ...
  list.forEach((e, i) => {
    ...
    const distance = (e.distance * 0.000621371192).toFixed(1);
    // If necessary, replace blank business image with placeholder
    const imgUrl = (!e.image_url) ? '/public/img/blank.png' : e.image_url;
...
function checkAll() {
  const userId = localStorage.getItem('rv-nightlife-id') || null;
  $('.attend-link').each(function () {
    ajaxFunctions.ajaxRequest('GET', `/api/attend/${$(this)[0].id}/${userId}`, (res) => {
      ...
      const userAction = (results.attendees.includes(userId)) ? 'attending' : 'no';
      updateAttending({
...
function attend(link, interested) {
  const userId = localStorage.getItem('rv-nightlife-id');
